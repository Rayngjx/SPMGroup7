  ...
  const generateSchedule = useCallback(() => {
    ...
    for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
      ...
      const isWFH = approvedDates.some(
        (dateItem) => dateItem.date.toDateString() === currentDate.toDateString()
      )
    ...
    <PageContainer scrollable={true}>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-bold tracking-tight">Your Personal Schedule</h2>
              ...
              <ScrollArea className="h-[calc(100vh-250px)]">
                <ul className="space-y-2">
                  {approvedDates.map((date) => (
                    <li key={date.request_id} className="flex justify-between items-center py-2 border-b last:border-b-0">
                      <span>{date.date.toLocaleDateString()}</span>
                      <span className="text-sm text-green-600 font-semibold">WFH</span>
                    </li>
                  ))}
                </ul>
              </ScrollArea>
